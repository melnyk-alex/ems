...
!function (e) {
    ...
    t.prototype = {
        listen: function () {
            ...
            if (t === "clear")return;
            this.$preview.text("");
            for (var iter in e.target.files) {
                if (!(e.target.files[iter] instanceof File)) continue;
                var n = e.target.files[iter];
                if (!n) {
                    this.clear();
                    return
                }
                ...
                if (this.type === "image" && this.$preview.length > 0 && (typeof n.type != "undefined" ? n.type.match("image.*") : n.name.match("\\.(gif|png|jpe?g)$")) && typeof FileReader != "undefined") {
                    ...
                    if (this.$preview.text().length > 0) this.$preview.text(this.$preview.text() + ", ");
                    this.$preview.text(this.$preview.text() + n.name), this.$element.addClass("fileupload-exists").removeClass("fileupload-new");
                }
            }
        }, clear: function (e) {
